...
/** Document keys the form owns; everything else is carried through untouched. */
const FORM_KEYS = ['proxy', 'noProxy', 'caFiles', 'caFile', 'insecure', 'debug', 'rules']

/** Rule keys the form owns. */
const RULE_KEYS = ['host', 'hosts', 'proxy', 'insecure', 'caFiles', 'caFile']

/** Everything in a document the form does not own, so a save cannot drop it. */
function carried(source, owned) {
  const rest = {}
  for (const [key, value] of Object.entries(source)) if (!owned.includes(key)) rest[key] = value
  return rest
}
 ...
 * @typedef {{ host: string, proxyMode: ProxyMode, proxy: string, insecure: boolean, caFiles: string,
 *   rest: Record<string, unknown> }} RuleForm
 ...
 * @returns {{ proxy: string, noProxy: string, caFiles: string, insecure: boolean, debug: boolean,
 *   rest: Record<string, unknown>, rules: RuleForm[] }} the form state.
 */
...
export function documentFromForm(form) {
  // Whatever the page does not edit — timeouts, settingsFile, a field a later
  // version adds — survives a save unchanged.
  const document = { ...form.rest }
  ...
  else delete document.insecure
  if (form.debug) document.debug = true
  else delete document.debug
  const rules = form.rules
    .map((rule) => {
      const hosts = splitList(rule.host)
      if (hosts.length === 0) return undefined
      const entry = { ...rule.rest, host: hosts.length === 1 ? hosts[0] : hosts }
...
export function emptyRule() {
  return { host: '', proxyMode: 'inherit', proxy: '', insecure: false, caFiles: '', rest: {} }
}
