...
import { Agent, Pool, ProxyAgent, getGlobalDispatcher, setGlobalDispatcher } from './vendor/undici.js'
...
/** One proxy URL without its credentials, for a log line. */
 ...
 */
export function buildTransport(policy, baseline, log) {
  const extraCertificates = readCertificates(trustedCaFiles(policy))
  const trust = [...baseline.trust, ...extraCertificates]
  // Only the timeouts the document names are passed on; everything else keeps
  // undici's default rather than this plugin inventing one.
  const limits = {
    ...policy.timeouts.headers === undefined ? {} : { headersTimeout: policy.timeouts.headers },
    ...policy.timeouts.body === undefined ? {} : { bodyTimeout: policy.timeouts.body },
  }
  const connectLimit = policy.timeouts.connect === undefined ? {} : { timeout: policy.timeouts.connect }
  ...
  const dispatcher = new Agent({
    factory(origin, options) {
      const address = typeof origin === 'string' ? origin : origin.origin
      const route = routeFor(policy, address)
      // The factory runs once per origin, so this line answers the question a
      // timing out request raises: did the policy see this origin at all, and
      // what did it decide for it.
      if (policy.debug) {
        log?.info(`route ${address} => ${route.proxy === null ? 'direct' : `proxy ${redactProxy(route.proxy)}`}${route.insecure ? ', verification off' : ''}`)
      }
      if (route.proxy !== null) {
        return new ProxyAgent({
          uri: route.proxy,
          ...limits,
          ...(Object.keys(connectLimit).length > 0 ? { proxyTls: { ...connectLimit }, requestTls: { ...connectLimit } } : {}),
          ...(route.insecure ? { requestTls: { ...connectLimit, rejectUnauthorized: false } } : {}),
          ...(extraCertificates.length > 0 && !route.insecure ? { requestTls: { ...connectLimit, ca: trust } } : {}),
        })
      }
      const connect = { ...(typeof options.connect === 'object' && options.connect !== null ? options.connect : {}), ...connectLimit }
      if (route.insecure) connect.rejectUnauthorized = false
      else if (extraCertificates.length > 0) connect.ca = trust
      return new Pool(origin, { ...options, ...limits, connect })
    },
  })
  const release = async () => {
...
export function apply(ctx, config) {
  ...
  const reload = (reason) => {
    ...
    try {
      ...
      transport = buildTransport(policy, baseline, log)
    } catch (cause) {
